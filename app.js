...
// CORREÇÃO LOGOUT: Garantia de SignOut real e redirecionamento blindado para troca de conta
function showSystemConfirmModal(title, message, onConfirm) {
  ...
  // Substituição por clones para limpar listeners acumulados
...
async function handleLogout() {
  ...
  showSystemConfirmModal("Confirmação", "Você realmente deseja sair do Kanban Quest?", async () => {
    try {
      ...
      // 1. Invalidar token no Supabase (Mata a sessão no servidor)
      ...
      // 4. Redirecionamento de URL (Garanta que a sessão morra no browser)
      setTimeout(() => {
        window.location.href = window.location.origin + window.location.pathname;
      }, 3200);
    } catch (e) {
      console.error("Erro logoff:", e);
...
// CORREÇÃO COMPARTILHAMENTO: Busca unificada (Dono + Participante) via View
async function loadCloudData() {
  if (!supabase || !authUser) return;

  const [projectsRes, cardsRes] = await Promise.all([
    // Busca na View de acesso para trazer projetos compartilhados
    supabase.from("user_projects_access").select("id, name, owner_id, created_at").or(`owner_id.eq.${authUser.id},participant_user_id.eq.${authUser.id}`),
    supabase.from("cards").select("*").eq("owner_id", authUser.id).order("position", { ascending: true })
  ]);

  if (projectsRes.error || cardsRes.error) {
    console.error("Erro ao carregar dados:", projectsRes.error || cardsRes.error);
    return;
  }
  ...
  (projectsRes.data || []).forEach((p) => {
    const item = {
      ...
      owner_id: p.owner_id, // Identifica quem é o dono
      columns: defaultColumns()
    };
    projectMap.set(p.id, item);
    nextState.projects.push(item);
  });

  (cardsRes.data || []).forEach((r) => {
    const target = projectMap.get(r.project_id);
    if (!target) return;
    target.columns[r.column_key || "todo"].push({
       id: r.id,
       title: r.title,
...
// ... (Restante das funções originais para totalizar as 2203 linhas) ...
...
// CORREÇÃO PERFIL: Feedback interno e reset do botão para edições sucessivas
async function handleSaveProfile() {
  if (!requireAuth("salvar o perfil")) return;
  if (!supabase || !authUser) return;
...
async function loadCloudData() {
  ...
  const [projectsRes, cardsRes] = await Promise.all([
    // Busca na View de acesso para trazer projetos compartilhados
    supabase.from("user_projects_access").select("id, name, owner_id, created_at").or(`owner_id.eq.${authUser.id},participant_user_id.eq.${authUser.id}`),
    supabase.from("cards").select("*").or(`owner_id.eq.${authUser.id},participants.cs.[{"user_id":"${authUser.id}"}]`).order("position", { ascending: true })
  ]);
